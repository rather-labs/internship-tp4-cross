...
async function getReceiptsAndProofs(messages, publicClient) {
  ...
  for (const msg of messages) {
    ...
    if (
      toHex(await verifyProof(proof, msg.txIndex)) !=
...
async function sendMsgs(
  ...
  for (let i = 0; i < data[chain].outgoingMsgs.length; i++) {
    if (data[chain].outgoingMsgs[i].finalityBlocks > blockNumber) {
      break;
    }

    if (data[chain].outgoingMsgs[i].fee < data[chain].currentFee) {
      continue;
    }
    ...
    if (data[chain].outgoingMsgs[i].taxi) {
      messages[data[chain].outgoingMsgs[i].destinationBC].push(
        data[chain].outgoingMsgs[i]
      );
    } else {
      data[chain].outgoingMsgsBus[
        data[chain].outgoingMsgs[i].destinationBC
      ].push(data[chain].outgoingMsgs[i]);
    }
  }

  for (const destinationBC of destinationBCs) {
    if (data[chain].outgoingMsgsBus[destinationBC].length >= BUS_CAPACITY) {
      messages[destinationBC] = concat(
        messages[destinationBC],
        data[chain].outgoingMsgsBus[destinationBC]
      );
    }

    const [receipts, proofs] = await getReceiptsAndProofs(
      messages[destinationBC],
      publicClients[chain]
    );
...
async function collectPayment(
  ...
  for (const sourceBC in data[chain].msgsToClaimPay) {
    for (let i = 0; i < data[chain].msgsToClaimPay[sourceBC].length; i++) {
      if (data[chain].finalityBlocksPay[sourceBC][i] > blockNumber) {
        break;
      }
...
async function handleNewMsg(log, event, { data, chain }) {
  console.log(
    `Msg emmited from ${data[chain].name}`,
    event.eventName,
    "Msg:",
    event.args.messageNumber
  );
  const outMsg = {
  ...
  const index = data[chain].outgoingMsgs.findIndex(
    (x) => x.finalityBlock >= outMsg.finalityBlock
  );
  if (index === -1) {
...
function handleUpdateFee(log, event, { data, chain }) {
  console.log(
    `Update fees for msg of ${data[chain].name}:`,
    event.eventName,
    "Msg:",
    event.args.messageNumber
  );
...
function handleMsgReceived(log, event, { data, chain }) {
  ...
  console.log(
    `Msg Received on ${chain}:`,
    event.eventName,
    "Msgs:",
    event.args.inboundMessageNumbers,
    "Succcess:",
    event.args.successfullInbound,
    "Failure Reasons:",
    event.args.failureReasons
  );
  ...
  for (let i = 0; i < event.args.inboundMessageNumbers.length; i++) {
    ...
    if (event.args.successfullInbound[i]) {
      const index = data[
        CHAIN_NAMES[event.args.sourceBC]
      ].outgoingMsgs.findIndex(
        (msg) =>
          msg.number == event.args.inboundMessageNumbers[i] &&
      ...
      if (index >= 0) {
        ...
        const index = data[CHAIN_NAMES[event.args.sourceBC]].outgoingMsgsBus[
        ...
        if (index >= 0) {
          ...
          data[CHAIN_NAMES[event.args.sourceBC]].outgoingMsgsBus[
            event.destinationChain
          ].splice(index, 1);
        }
      }
    ...
    if (
      event.args.relayer == EXTERNAL_ADDRESSES[CHAIN_NAMES[event.args.sourceBC]]
    ) {
...
const events = [
  ...
  ["outgoing", "UpdateMessageFee"],
  ["incoming", "InboundMessagesRes"],
]; // ['outgoing', 'OutboundMessage'] / ['outgoing', 'UpdateMessageFee'] /  ['incoming', 'InboundMessage']
// Functions for event handling, must be in the same order as events
const eventsHandlingFunctions = [
  handleNewMsg,
  handleUpdateFee,
  handleMsgReceived,
];
...
function setup_relayer(
  ...
  blockChains.forEach((chain) => {
    publicClients[chain] = initializePublicClient({
      chain,
      rpc: RPCProviders[0],
    });
    walletClients[chain] = initializeWalletClient({
      chain,
      rpc: RPCProviders[0],
    });
    outgoingMsgsBus = {};
    blockChains.forEach((chain) => {
      outgoingMsgsBus[CHAIN_IDS[chain]] = [];
    });
    ...
    listenForNewBlocks(publicClients[chain], newBlockHandlingFunction, {
      data,
      chain,
      publicClients,
      walletClients,
